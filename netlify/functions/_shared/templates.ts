...
// Dates arrive as ISO strings from the frontend (e.g. "2026-05-11")
function formatDate(iso: string | null | undefined): string {
  if (!iso) return '—'
  try {
    // Append noon time so local-date parsing doesn't roll back a day
...
function productsTable(products: Product[]): string {
  const rows = products
    .map(
      (p) => `<tr style="border-top:1px solid #e5e7eb">
        <td style="padding:8px 12px;font-size:14px;color:#111827">${p.name}</td>
        <td style="padding:8px 12px;text-align:right;font-size:14px;font-weight:600;color:#111827">x${p.quantity}</td>
      </tr>`,
    )
    .join('')
...
function ctaButton(text: string, url: string): string {
  return `<p style="text-align:center;margin:24px 0 0">
    <a href="${url}"
      style="display:inline-block;background:#00C2FF;color:#0a1628;font-weight:700;
...
export function tplAdminNuevoPedido(order: {
  ...
  return layout(`
    <p style="margin:0 0 20px;font-size:16px;font-weight:600">Nuevo pedido recibido</p>
