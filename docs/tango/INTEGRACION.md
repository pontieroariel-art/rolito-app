...
**Encendido completo el 2026-09-17 a la noche (22:00–23:00, Ariel guiado paso a paso):** bridge nuevo en la VM; `DIF` "DIF DE REPARTO" (transferencia, talonario 13, ID_STA13 151) creado en Tango copiando CAR; los tres tipos cargados; functions desplegadas; `--probar-sql` OK; dry-run de cada sentido contra el Tango real con la transacción revertida (merma: descarga de Mira, CAM 06 → 99; diferencia: liquidación de prueba LQ-03-000001, DIF 03 → 98; cambio de ventanilla: item inventado sobre una venta de ADDON, CAM 01 → 99) y `habilitado=true` en los tres más `ventaPromo.incluyeCambios=false`. Primeros comprobantes reales: **CAM 0000100000111** (Primiterra, 21 → 99, 41 × 3 kg + 5 × 10 kg) y **CAM 0000100000112** (Mira, 06 → 99, 4 × 3 kg), encolados a mano con `scripts/tango/encolar-merma-fase-b.mjs` porque esas descargas se contaron antes del deploy. Los items de prueba se arman con `scripts/tango/encolar-prueba-fase-b.mjs` (`--diferencia <liquidacionId>` / `--cambio-ventanilla <ventaId>`): nacen en `estado: error` con la nota "PRUEBA EN SECO", el barrido normal nunca los toma y solo los ve `--solo`. Transición asumida: las rotas de los viajes del 16 y 17 se descuentan dos veces del camión (sus ventas promo ya habían descontado los cambios); el 99 queda bien y los depósitos de los camiones ya estaban lejos de la realidad (camión 06 en −11.719 bolsas de 3 kg; camión 03 en −8.707 de 2 kg) a la espera de la regularización. Las escrituras en config y cola de producción las corre Ariel con `!` (el permiso de la sesión de Claude las bloquea).
