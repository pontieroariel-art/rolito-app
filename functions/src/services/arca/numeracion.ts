...
// Interfaces mínimas de Firestore, para poder testear sin levantar el emulador.
// Coinciden con la forma de firebase-admin, así que el llamador le pasa el db real.
...
export interface ClaveNumeracion {
  ptoVta: number
  cbteTipo: number
}
 ...
 */
export function rutaContador({ ptoVta, cbteTipo }: ClaveNumeracion): string {
  return `config/arcaNumeracion_${ptoVta}_${cbteTipo}`
}

export interface EstadoContador {
  ultimoAsignado: number
  /** Números reservados que quedaron sin usar y hay que consumir antes de seguir. */
  librados: number[]
}

function leerEstado(snap: SnapshotLike): EstadoContador | null {
  if (!snap.exists) return null
  const d = snap.data() ?? {}
  const ultimoAsignado = typeof d.ultimoAsignado === 'number' ? d.ultimoAsignado : null
  if (ultimoAsignado === null) return null
  return {
    ultimoAsignado,
    librados: Array.isArray(d.librados) ? (d.librados as number[]).filter((n) => typeof n === 'number') : [],
  }
}
...
export async function inicializarContador(
  db: DbLike,
  clave: ClaveNumeracion,
  obtenerUltimoDeArca: () => Promise<number>,
): Promise<EstadoContador> {
  ...
  return db.runTransaction(async (tx) => {
    ...
    const estado: EstadoContador = { ultimoAsignado: ultimoEnArca, librados: [] }
 ...
 */
export async function reservarNumero(db: DbLike, clave: ClaveNumeracion): Promise<number> {
  const ref = db.doc(rutaContador(clave))
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (!estado) {
      throw new Error(
        `El contador ${rutaContador(clave)} no está inicializado. ` +
 ...
 * problema fiscal.
 */
export async function marcarNumeroLibre(
  db: DbLike,
  clave: ClaveNumeracion,
  numero: number,
): Promise<void> {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (estado.librados.includes(numero)) return   // ya estaba liberado
...
export async function verificarSincronizacion(
  ...
  clave: ClaveNumeracion,
  obtenerUltimoDeArca: () => Promise<number>,
): Promise<{ enSync: boolean; local: number | null; arca: number }> {
  const ref = db.doc(rutaContador(clave))
