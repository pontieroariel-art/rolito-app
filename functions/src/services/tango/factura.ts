...
export function armarNotaCreditoFacturador(payload: PayloadVenta, item: ItemOutbox, cfg: ConfigFacturadorEmpresa, mapeos: MapeosFactura): ArmadoFactura {
  if (payload.notaCreditoInterna && !payload.notaCredito) return armarNotaCreditoInternaFacturador(payload, item, cfg, mapeos)
 ...
 */
export function armarNotaCreditoInternaFacturador(payload: PayloadVenta, item: ItemOutbox, cfg: ConfigFacturadorEmpresa, mapeos: MapeosFactura): ArmadoFactura {
  const nci = payload.notaCreditoInterna
  if (!nci || typeof nci.numero !== 'number' || typeof nci.puntoVenta !== 'number') return { error: 'La anulación no tiene una nota de crédito interna numerada (notaCreditoInterna.puntoVenta/numero)' }
  const numeroFactura = String(payload.tango?.facturaNumero ?? '').trim()
  if (!numeroFactura) return { error: 'La factura de promo no está registrada en Tango (tango.facturaNumero): no hay a qué referenciar la NC' }
  const base = armarComprobanteFacturador(payload, item, cfg, mapeos)
  if (base.error !== undefined) return base
  if (base.fiscal) return { error: 'La venta tiene factura con CAE: su anulación va por nota de crédito de ARCA, no interna' }
  const letra = String(mapeos.letraNoFiscal ?? '')
  if (!letra) return { error: 'Falta la letra (A/B) con la que Tango registra la promo (categoría de IVA del cliente)' }
  const empresa = item.empresa ?? '?'
  const talonario = cfg.talonariosNC?.[letra]
  if (!talonario) return { error: `Falta config/tango.facturador.${empresa}.talonariosNC.${letra} (talonario de nota de crédito ${letra})` }
