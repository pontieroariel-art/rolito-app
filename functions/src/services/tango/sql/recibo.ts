...
export async function leerDatosRecibo(db: EjecutorSql, r: ReciboTango, cfg: ConfigReciboSql, identity: Set<string>): Promise<DatosRecibo> {
  ...
  const c = cli[0]
  if (!c) throw new Error(`cliente ${r.codCliente} no existe en Tango`)
  ...
  for (const imp of r.imputaciones) {
    ...
    const fac = f[0]
    if (!fac) throw new Error(`la factura ${imp.tComp} ${imp.nComp} no existe en Tango`)
    if (fac.COD_CLIENT.trim() !== r.codCliente) throw new Error(`la factura ${imp.nComp} es del cliente ${fac.COD_CLIENT}, no de ${r.codCliente}`)
  ...
  for (const a of r.aplicaciones) {
    ...
    const rec = q[0]
    if (!rec) throw new Error(`el recibo a cuenta ${a.nComp} no existe en Tango`)
    if (rec.COD_CLIENT.trim() !== r.codCliente) throw new Error(`el recibo a cuenta ${a.nComp} es del cliente ${rec.COD_CLIENT}, no de ${r.codCliente}`)
    if (String(rec.ESTADO).trim() === 'ANU') throw new Error(`el recibo a cuenta ${a.nComp} está anulado en Tango`)
  ...
  for (const cod of [cfg.cuentas.contracuenta, ...r.medios.map((m) => m.cuenta)]) {
    ...
    const cta = q[0]
    if (!cta) throw new Error(`la cuenta de tesorería ${cod} no existe en Tango (SBA01)`)
  ...
  if (r.cheques.length) {
    ...
    for (const ch of r.cheques) {
      ...
      if (idBanco == null) {
        ...
        const banco = b[0]
        if (!banco) throw new Error(`el banco ${ch.bancoCodigo} del cheque ${ch.numero} no existe en Tango (${tabla}.${col}); cargarlo o mapearlo en config/tango.sql.recibo.cheques.bancos`)
