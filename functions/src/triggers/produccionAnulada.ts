...
export type AccionAnulacionPallet = 'nada' | 'descartar' | 'avisarOficina'

/** Qué hacer con el item de la cola de un pallet que se acaba de anular. Pura. */
export function accionAnulacionPallet(estadoCola: string | null | undefined): AccionAnulacionPallet {
  if (!estadoCola) return 'nada'                                   // nunca se encoló
  if (estadoCola === 'pendiente' || estadoCola === 'error') return 'descartar'
  if (estadoCola === 'descartado') return 'nada'
  return 'avisarOficina'                                           // enviado / confirmado
}

/** Texto de la push a la oficina. Pura. */
export function avisoPalletAnulado(p: { codigo?: string; productoNombre?: string; unidades?: number; anulacion?: { motivo?: string; por?: { nombre?: string } } }, numeroTango?: string | null): { titulo: string; cuerpo: string } {
  return {
...
export const onProduccionPalletAnulado = onDocumentUpdated(
  { document: 'produccionPallets/{palletId}', secrets: [vapidPublicKey, vapidPrivateKey] },
  async (event) => {
    const antes = event.data?.before.data()
    const despues = event.data?.after.data()
    if (!despues || antes?.anulacion || !despues.anulacion) return

    const db = getFirestore()
    const colaRef = db.doc(`tango-outbox/produccionPallets_${event.params.palletId}`)
    const cola = await colaRef.get()
    const estado = cola.exists ? (cola.data()?.estado as string | undefined) : null
    ...
    if (accion === 'descartar') {
      ...
      const descartado = await db.runTransaction(async (tx) => {
        ...
        const e = snap.data()?.estado as string | undefined
        if (e !== 'pendiente' && e !== 'error') return false
        tx.update(colaRef, { estado: 'descartado', ultimoError: 'pallet anulado en la app antes de ir a Tango', actualizadoEn: FieldValue.serverTimestamp() })
        return true
      })
    ...
    const resultado = (await colaRef.get()).data()?.resultado as { produccionNumero?: string } | undefined
    await colaRef.set({ anulacionApp: { estado: 'pendiente_oficina', en: FieldValue.serverTimestamp() } }, { merge: true })
    try {
      ...
      await enviarPushAUsuarios(destinatarios.docs, { titulo, cuerpo, url: '/produccion/listado' }, { vapidPublicKey: vapidPublicKey.value(), vapidPrivateKey: vapidPrivateKey.value() })
    } catch (e) {
      console.error(`[produccion] push de pallet anulado falló: ${(e as Error).message}`)
    }
  },
)
