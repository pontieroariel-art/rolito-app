...
export interface ResumenClientesEmpresa {
  ...
  lotes: number
  actualizados: number
  /** Fichas iguales que no se reescribieron (2026-09-26). */
  sinCambios?: number
...
export async function sincronizarSaldos(db: Firestore, tango: TangoClient, cfg: ConfigSync): Promise<ResumenSaldos> {
  ...
  for (const empresa of EMPRESAS) {
    ...
    try {
      ...
      logger.error(`[tango] sync de saldos de ${empresa} falló: ${re.error}`, { lotes: re.lotes, stack: (e as Error).stack })
    }
...
export const syncClientesTangoConnect = onSchedule(
  { schedule: '0 5,7-19 * * *', timeZone: TZ, secrets: [tangoApiToken], timeoutSeconds: 540, memory: '512MiB', cpu: 0.5 },
  async () => {
