...
export const onDescargaMarcaRemito = onDocumentCreated('descargasCamion/{descargaId}', async (event) => {
  const d = event.data?.data()
  const remitoId = typeof d?.remitoId === 'string' ? d.remitoId.trim() : ''
  if (!d || !remitoId) return
  const ref = getFirestore().doc(`remitosCarga/${remitoId}`)
  const remito = await ref.get()
  if (!remito.exists) return
  const marca = {
