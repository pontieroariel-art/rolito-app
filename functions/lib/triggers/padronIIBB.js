...
exports.avisarPadronIIBB = exports.RUTA_PADRON = void 0;
exports.hoyEnAr = hoyEnAr;
exports.diasHasta = diasHasta;
exports.decidirAviso = decidirAviso;
/**
...
const DIAS_AVISO = 7;
exports.RUTA_PADRON = 'config/arcaPadronIIBB';
/** Día calendario argentino en formato YYYY-MM-DD. */
function hoyEnAr(ahora) {
 ...
 */
function decidirAviso(estado, hoy) {
    if (!estado?.vigenciaHasta)
        return { tipo: 'sin-padron', dias: 0 };
    const dias = diasHasta(hoy, estado.vigenciaHasta);
    if (dias < 0)
        return { tipo: 'vencido', dias };
    if (dias > DIAS_AVISO)
        return null;
...
function cuerpo(aviso, estado) {
    ...
    <p style="margin-top:18px;color:#6E6C63;font-size:13px">
      ...
      <a href="${email_1.APP_URL}" style="color:#1D9E75">Rolito</a>
    </p>`;
...
exports.avisarPadronIIBB = (0, scheduler_1.onSchedule)({ schedule: '0 9 * * *', timeZone: TZ, secrets: [email_1.resendApiKey] }, async () => {
    ...
    await (0, email_1.sendEmail)(emails, asunto, html);
    // Deja constancia para no repetir el aviso de "por vencer" todos los días.
    await db.doc(exports.RUTA_PADRON).set({ ultimoAviso: { tipo: aviso.tipo, fecha: hoy }, avisadoEn: firestore_1.FieldValue.serverTimestamp() }, { merge: true });
    console.log(`[avisarPadronIIBB] aviso "${aviso.tipo}" enviado a ${emails.length} destinatario(s)`);
});
//# sourceMappingURL=padronIIBB.js.map