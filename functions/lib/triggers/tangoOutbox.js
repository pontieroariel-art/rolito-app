 ...
 */
async function payloadDeVentaEn(venta, empresa) {
    ...
    if (codigo)
        payload.clienteCodigoTango = codigo;
    return payload;
}
/**
...
async function encolarVenta(coleccion, ventaId, venta) {
    ...
    if (!destino) {
        ...
        console.warn(`[tango] la venta ${coleccion}/${ventaId} no dice a dónde va ` +
    ...
    const stock = (0, circuito_1.movimientoStockDeVenta)(venta.canal, venta.formaPago, venta.total);
    if (stock) {
...
exports.onCobranzaCreada = (0, firestore_1.onDocumentCreated)('cobranzas/{cobranzaId}', async (event) => {
    ...
    const empresa = (0, empresas_1.esEmpresa)(cobranza.empresa) ? cobranza.empresa : 'redonhielo';
    ...
    const idsEmpresa = (user ? (0, empresas_1.tangoIdsDe)(user)[empresa] : undefined) ?? [];
...
const WRITE_BACKS = {
    ...
    produccionPallet: {
        ...
        buildUpdate: (resultado) => {
            ...
            if (!numero)
                return null;
            return { 'tango.estado': 'confirmado', 'tango.numero': String(numero).trim(), 'tango.ultimoError': firestore_2.FieldValue.delete() };
        },
        buildError: (ultimoError) => ({ 'tango.estado': 'error', 'tango.ultimoError': ultimoError }),
    },
    ...
    anulacionRemito: {
        ...
        buildUpdate: (resultado) => {
            ...
            return {
                ...
                'anulacion.tango.en': firestore_2.FieldValue.serverTimestamp(),
            };
        },
