...
exports.resetearPasswordStaff = void 0;
...
exports.resetearPasswordStaff = (0, https_1.onCall)(async (request) => {
    if (!request.auth)
    ...
    if (!admin || admin.rol !== 'super_admin' || admin.estado !== 'activo') {
        throw new https_1.HttpsError('permission-denied', 'Solo el super_admin restablece contraseñas');
    }
    await (0, rateLimit_1.assertRateLimit)(adminUid, 'resetearPasswordStaff', 30, 3600);
    const { uid } = (request.data ?? {});
    if (typeof uid !== 'string' || uid.length === 0 || uid.length > 128) {
        throw new https_1.HttpsError('invalid-argument', 'Falta el usuario');
    }
    const snap = await db.collection('users').doc(uid).get();
    if (!snap.exists)
        throw new https_1.HttpsError('not-found', 'No existe ese usuario');
    const perfil = snap.data() ?? {};
    const rol = String(perfil.rol ?? '');
    if (rol === 'super_admin') {
        throw new https_1.HttpsError('permission-denied', 'No se restablece la contraseña de otro super_admin');
    }
    if (rol === 'cliente') {
        throw new https_1.HttpsError('failed-precondition', 'Los clientes recuperan su contraseña por mail');
    }
    if (rol === 'produccion_hielo') {
        throw new https_1.HttpsError('failed-precondition', 'Los operarios de producción se resetean desde Producción');
    }
    ...
    if (rol === 'chofer' || rol === 'tecnico') {
        throw new https_1.HttpsError('failed-precondition', 'Ese usuario entra con PIN, no con contraseña');
    }
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', 'No se pudo restablecer la contraseña. Intentá de nuevo.');
    }
