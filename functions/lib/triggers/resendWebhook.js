...
exports.resendWebhook = exports.pisaEstado = void 0;
exports.firmaValida = firmaValida;
exports.estadoDeEvento = estadoDeEvento;
...
const webhookSecret = (0, params_1.defineSecret)('RESEND_WEBHOOK_SECRET');
...
function firmaValida(secret, svixId, svixTimestamp, svixSignature, cuerpo, ahoraSeg = Math.floor(Date.now() / 1000)) {
    if (!secret || !svixId || !svixTimestamp || !svixSignature)
        return false;
    const ts = Number(svixTimestamp);
    if (!Number.isFinite(ts) || Math.abs(ahoraSeg - ts) > TOLERANCIA_SEG)
        return false;
    const clave = Buffer.from(secret.replace(/^whsec_/, ''), 'base64');
    const esperada = (0, crypto_1.createHmac)('sha256', clave).update(Buffer.concat([Buffer.from(`${svixId}.${svixTimestamp}.`), cuerpo])).digest();
    for (const parte of svixSignature.split(' ')) {
        const [version, firma] = parte.split(',');
        if (version !== 'v1' || !firma)
            continue;
        const recibida = Buffer.from(firma, 'base64');
        if (recibida.length === esperada.length && (0, crypto_1.timingSafeEqual)(recibida, esperada))
            return true;
    }
    return false;
}
/** Qué estado de la app corresponde a cada evento; `null` = no interesa (sent, opened, clicked). */
function estadoDeEvento(tipo) {
    switch (tipo) {
...
const RANGO = { aceptado: 0, demorado: 1, entregado: 2, queja: 3, rebotado: 3 };
const pisaEstado = (actual, nuevo) => (RANGO[nuevo] ?? 0) >= (RANGO[actual ?? 'aceptado'] ?? 0);
exports.pisaEstado = pisaEstado;
const detalleDe = (e, estado) => {
    const d = e.data;
    if (estado === 'rebotado') {
        const b = d?.bounce;
        const partes = [b?.type, b?.subType, b?.message, d?.failed?.reason].filter((x) => typeof x === 'string' && x.length > 0);
        return partes.join(' · ').slice(0, 300) || 'El servidor del destinatario rechazó el mail';
    }
    if (estado === 'queja')
        return 'El destinatario lo marcó como correo no deseado';
    if (estado === 'demorado')
        return 'El servidor del destinatario todavía no lo aceptó; se reintenta';
    return '';
};
exports.resendWebhook = (0, https_1.onRequest)({ secrets: [webhookSecret, vapidPublicKey, vapidPrivateKey], invoker: 'public' }, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).send('method not allowed');
        return;
    }
    const h = (n) => String(req.headers[n] ?? '');
    const cuerpo = req.rawBody ?? Buffer.from(JSON.stringify(req.body ?? {}));
    ...
    if (!estado || !emailId) {
        res.status(200).json({ ok: true, ignorado: evento.type ?? 'sin tipo' });
        return;
    }
    const db = (0, firestore_1.getFirestore)();
    const ref = db.collection('mailsSalientes').doc((0, email_1.idMailSaliente)('resend', emailId));
    const svixId = h('svix-id');
    const detalle = detalleDe(evento, estado);
    const entrega = { estado, detalle, en: firestore_1.FieldValue.serverTimestamp(), evento: evento.type, svixId };
    try {
        const snap = await ref.get();
        const actual = snap.data();
        if (actual?.entrega?.svixId === svixId) {
            res.status(200).json({ ok: true, repetido: true });
            return;
        }
        if (actual && !(0, exports.pisaEstado)(actual.estado, estado)) {
            res.status(200).json({ ok: true, conservado: actual.estado });
            return;
        }
        ...
        await ref.set({
            ...
            ...(actual ? {} : { proveedor: 'resend', mailId: emailId, para, asunto: String(evento.data?.subject ?? '').slice(0, 200), tipo: 'aviso', fecha: firestore_1.FieldValue.serverTimestamp() }),
            estado,
            entrega,
        }, { merge: true });
        ...
        await Promise.all(referencias.map(async (r) => {
            try {
                await db.doc(`${r.coleccion}/${r.id}`).update({ [r.campo]: { estado, detalle, en: firestore_1.FieldValue.serverTimestamp() } });
            }
            catch (e) {
                console.warn(`[resendWebhook] no se pudo anotar ${r.coleccion}/${r.id}.${r.campo}: ${e.message}`);
            }
        }));
        if (estado === 'rebotado' || estado === 'queja') {
            try {
                ...
                console.error(`[resendWebhook] push a facturación falló: ${e.message}`);
            }
        }
