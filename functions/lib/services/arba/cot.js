...
const aaaammdd = (d) => `${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, '0')}${String(d.getDate()).padStart(2, '0')}`;
...
const centesimos = (n) => String(Math.round(n * 100));
exports.centesimos = centesimos;
/** CODIGO_UNICO del comprobante respaldatorio: código AFIP (3) + prefijo (5) + número (8). */
const codigoUnicoComprobante = (codigoComprobante, prefijo, numero) => `${digitos(codigoComprobante).padStart(3, '0').slice(-3)}${String(prefijo).padStart(5, '0').slice(-5)}${String(numero).padStart(8, '0').slice(-8)}`;
exports.codigoUnicoComprobante = codigoUnicoComprobante;
/** Nombre del archivo: TB_<cuit>_<planta><puerta>_<aaaammdd>_<secuencia>.txt */
const nombreArchivoCot = (cuit, planta, fecha, secuencia) => `TB_${digitos(cuit)}_${planta.codigoPlanta.padStart(3, '0')}${planta.puerta.padStart(3, '0')}_${aaaammdd(fecha)}_${String(secuencia).padStart(6, '0')}.txt`;
exports.nombreArchivoCot = nombreArchivoCot;
...
function armarArchivoCot(remito, sol, cfg, secuencia) {
    ...
    if (cuit.length !== 11)
        throw new Error('config/cot.cuit tiene que tener 11 dígitos');
    ...
    for (const it of remito.items) {
        ...
        if (!p || !(p.pesoKg > 0))
            throw new Error(`Falta el peso por unidad en config/cot.productos para "${it.nombre}" (${it.productoId})`);
        if (!/^\d{6}$/.test(p.codigoArba))
            throw new Error(`Código de ARBA inválido para "${it.nombre}": ${p.codigoArba}`);
        const kg = it.cantidad * p.pesoKg;
        kgTotal += kg;
        productos.push(['03', p.codigoArba, '1', (0, exports.centesimos)(kg), campo(it.productoId, 25), campo(p.descripcion || it.nombre, 40), 'KILOGRAMOS', (0, exports.centesimos)(kg)].join('|'));
    }
    if (!productos.length)
        throw new Error('El remito de carga no tiene productos con cantidad');
    const d = sol.destino;
    ...
    const remitoLinea = [
        ...
        (0, exports.codigoUnicoComprobante)(sol.respaldo.codigoComprobante, sol.respaldo.prefijo, sol.respaldo.numero), // CODIGO_UNICO
        ...
        importe > 0 ? (0, exports.centesimos)(importe) : '0', // IMPORTE (12+2)
    ].join('|');
    const lineas = [`01|${cuit}`, remitoLinea, ...productos, '04|1'];
    return {
        nombre: (0, exports.nombreArchivoCot)(cuit, planta, remito.fechaEmision, secuencia),
...
function parsearRespuestaCot(xml) {
    ...
    if (cot && (procesado ?? 'SI').toUpperCase() === 'SI')
        return { ok: true, ...base, ...(errores.length ? { detalle: errores } : {}) };
