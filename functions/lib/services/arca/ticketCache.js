...
exports.MARGEN_RENOVACION_MS = void 0;
exports.rutaTicket = rutaTicket;
exports.obtenerTicketAcceso = obtenerTicketAcceso;
exports.invalidarTicket = invalidarTicket;
 ...
 */
exports.MARGEN_RENOVACION_MS = 10 * 60 * 1000;
function rutaTicket(ambiente, servicio, cuit) {
    return `arcaTickets/${ambiente}_${servicio}_${cuit}`;
}
function leerTicket(data, ahora) {
    if (!data)
        return null;
    const { token, sign, expiracionMs } = data;
    if (typeof token !== 'string' || typeof sign !== 'string' || typeof expiracionMs !== 'number') {
        return null;
    }
    if (expiracionMs - exports.MARGEN_RENOVACION_MS <= ahora.getTime())
        return null; // vencido o por vencer
 ...
 */
async function obtenerTicketAcceso(opts) {
    const { db, cuit, ambiente } = opts;
    const servicio = opts.servicio ?? wsaa_1.SERVICIO_WSFE;
    const ahora = opts.ahora ?? new Date();
    const solicitar = opts.solicitar ?? wsaa_1.solicitarTicketAcceso;
    const ref = db.doc(rutaTicket(ambiente, servicio, cuit));
    const leerDelCache = () => db.runTransaction(async (tx) => leerTicket((await tx.get(ref)).data(), ahora));
    const cacheado = await leerDelCache();
    if (cacheado)
        return cacheado;
    const guardar = (ta) => db.runTransaction(async (tx) => {
        tx.set(ref, {
            token: ta.token,
            sign: ta.sign,
    ...
    try {
        ...
        if (!/ya posee un TA valido/i.test(mensaje))
            throw e;
 ...
 */
async function invalidarTicket(db, ambiente, cuit, servicio = wsaa_1.SERVICIO_WSFE) {
    const ref = db.doc(rutaTicket(ambiente, servicio, cuit));
    await db.runTransaction(async (tx) => {
        tx.set(ref, { token: null, sign: null, expiracionMs: 0 }, { merge: true });
    });
}
//# sourceMappingURL=ticketCache.js.map