...
function ProgramaForm({
  initial, choferes, clientes, onSave, onCancel,
}: {
  ...
  return (
    <div className="space-y-4">
          ...
          {DOW_LABELS.map((label, i) => (
            <button key={i} type="button" onClick={() => toggleDia(i)}
              className={`w-10 h-10 rounded-full text-sm font-bold border transition-colors ${
                dias.includes(i) ? 'bg-accent text-white border-accent' : 'bg-white border-[#D3D1C7] text-gray-500 hover:border-accent hover:text-gray-900'
              }`}>
              {label}
            </button>
          ))}
      ...
      <div>
        ...
        <select value={driverId} onChange={(e: ChangeEvent<HTMLSelectElement>) => setDriverId(e.target.value)}
          className="w-full bg-white border border-[#D3D1C7] rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-1 focus:ring-accent">
          <option value="">— Sin asignar —</option>
          {choferes.map((c) => <option key={c.uid} value={c.email}>{choferLabel(c)}</option>)}
        </select>
      </div>
      <div>
        <label className="text-xs text-gray-500 mb-1 block">Notas internas</label>
        <textarea value={notas} onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setNotas(e.target.value)}
          rows={2} placeholder="Horario preferido, instrucciones..."
...
function VisitaPuntualForm({
  clientes, choferes, defaultDate, onSave, onCancel,
}: {
  clientes:    UserProfile[]
  choferes:    UserProfile[]
  defaultDate: string
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input type="date" value={fecha} onChange={(e: ChangeEvent<HTMLInputElement>) => setFecha(e.target.value)}
            className="w-full bg-white border border-[#D3D1C7] rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-1 focus:ring-accent" />
        </div>
        <div>
...
function VisitaCard({
  ...
  return (
    <div className={`bg-white border rounded-xl p-4 space-y-2 ${
      isVisitado ? 'border-green-200 opacity-70' : isSinContacto ? 'border-amber-300 opacity-70' : 'border-[#D3D1C7]'
    }`}>
      <div className="flex justify-between items-start gap-3">
        <div className="min-w-0">
          ...
          {chofer       && <p className="text-xs text-gray-500 mt-1">Chofer: <span className="text-gray-900">{choferLabel(chofer)}</span></p>}
        </div>
        <div className="flex gap-1 shrink-0">
          {onSinContacto && !isVisitado && !isSinContacto && (
            <button onClick={onSinContacto}
...
export default function VisitasPanel() {
  ...
  const sinVisitar       = visitasParaFecha(visitas, new Date()).filter((v) => v.status === 'pendiente').length

  if (loadP || loadV) return <div className="flex justify-center py-20"><LoadingSpinner /></div>
  ...
  return (
    <div className="space-y-4">
        ...
        {(['agenda', 'programas', 'seguimiento'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors -mb-px ${
              tab === t ? 'border-accent text-accent' : 'border-transparent text-gray-500 hover:text-gray-900'
            }`}>
            {t === 'agenda' ? (
        ...
        <div className="space-y-4">
          ...
          {programasHoy.length === 0 && visitasPuntuales.length === 0 ? (
            ...
            <div className="space-y-2">
              {programasHoy.map((p) => (
                <VisitaCard key={`prog-${p.id}`} clientName={p.clientName} clientAddress={p.clientAddress}
                  clientPhone={p.clientPhone} driverId={p.driverId} notas={p.notas} isRecurrente choferes={choferes} />
              ))}
        ...
        <div className="space-y-4">
          ...
          {programas.length === 0 ? (
            <div className="bg-white border border-[#D3D1C7] rounded-xl p-8 text-center">
              ...
              <p className="text-gray-400 text-xs mt-1">Agregá un programa para que las visitas aparezcan automáticamente cada semana</p>
            </div>
          ) : (
            <div className="space-y-2">
              {programas.map((p) => {
                ...
                return (
                  <div key={p.id} className={`bg-white border rounded-xl p-4 flex flex-wrap items-start justify-between gap-3 ${p.activo ? 'border-[#D3D1C7]' : 'border-gray-200 opacity-60'}`}>
                      ...
                      <div className="flex gap-1 mt-2">
                        {DOW_LABELS.map((label, i) => (
                          <span key={i} className={`w-6 h-6 rounded-full text-xs flex items-center justify-center font-bold ${
                            p.diasSemana.includes(i) ? 'bg-accent text-white' : 'bg-gray-100 text-gray-300'
                          }`}>{label}</span>
                        ))}
                      </div>
                      {chofer  && <p className="text-xs text-gray-500 mt-1.5">Chofer: <span className="text-gray-900">{choferLabel(chofer)}</span></p>}
                      {p.notas && <p className="text-xs text-gray-400 italic mt-1">"{p.notas}"</p>}
                    </div>
                    <div className="flex gap-2 shrink-0">
                      <button onClick={() => openEditPrograma(p)}
                        className="text-xs text-gray-500 hover:text-gray-900 border border-[#D3D1C7] hover:border-accent rounded-lg px-3 py-1.5 transition-colors">Editar</button>
                      <button onClick={() => updatePrograma(p.id, { activo: !p.activo })}
                        className="text-xs text-gray-500 hover:text-gray-900 border border-[#D3D1C7] hover:border-accent rounded-lg px-3 py-1.5 transition-colors">{p.activo ? 'Pausar' : 'Activar'}</button>
                      <button onClick={() => deletePrograma(p.id)}
                        className="text-xs text-gray-500 hover:text-red-500 border border-[#D3D1C7] hover:border-red-300 rounded-lg px-3 py-1.5 transition-colors">✕</button>
                    </div>
                  </div>
                )
              })}
            ...
            if (visitaClientes.length === 0) {
              return (
                <div className="bg-white border border-[#D3D1C7] rounded-xl p-10 text-center">
                  ...
                  <p className="text-gray-400 text-xs mt-1">Marcá un cliente como visita desde Usuarios → Ficha del cliente</p>
                </div>
              )
            }
                ...
                <div className="space-y-2">
                  {visitaClientes
                    ...
                    .map((c) => {
                      ...
                      return (
                        <div key={c.uid} className={`bg-white border rounded-xl p-4 space-y-2 ${scheduled ? 'border-[#D3D1C7]' : 'border-amber-300'}`}>
                          <div className="flex justify-between items-start gap-3">
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                <p className="font-semibold text-sm text-gray-900">{clientLabel(c)}</p>
                                {c.frecuenciaVisita && (
                                  <span className="text-xs px-1.5 py-0.5 rounded bg-violet-100 text-violet-700 border border-violet-200">{FRECUENCIA_LABELS[c.frecuenciaVisita]}</span>
                                )}
                              </div>
                              {primaryAddr?.address && <p className="text-xs text-gray-500 mt-0.5 truncate">{primaryAddr.address}</p>}
                              {prog ? (
                                <div className="flex gap-1 mt-2">
                                  {DOW_LABELS.map((label, i) => (
      ...
      <Modal open={addProgramaModal} onClose={() => setAddProgramaModal(false)} title="Nuevo programa de visita" variant="light">
        {loadingClients ? <LoadingSpinner /> : (
          <ProgramaForm choferes={choferes} clientes={clientes}
      ...
      {editPrograma && (
        <Modal open onClose={() => setEditPrograma(null)} title="Editar programa" variant="light">
          {loadingClients ? <LoadingSpinner /> : (
            <ProgramaForm initial={editPrograma} choferes={choferes} clientes={clientes}
      ...
      <Modal open={addVisitaModal} onClose={() => setAddVisitaModal(false)} title="Agregar visita puntual" variant="light">
        {loadingClients ? <LoadingSpinner /> : (
          <VisitaPuntualForm clientes={clientes} choferes={choferes} defaultDate={agendaDate}
