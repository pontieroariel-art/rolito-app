  ...
  return (
    <section className="bg-white rounded-2xl border border-[#D3D1C7] shadow-sm p-5 space-y-5">
      ...
      <div className="grid sm:grid-cols-4 gap-3">
        ...
        <label className="block"><span className={label}>Lista de precios de Tango que valúa la carga (Redonhielo)</span><input value={form.listaPrecios} onChange={(e) => setForm({ ...form, listaPrecios: e.target.value.replace(/\D/g, '') })} className={input} placeholder="301 = Habituales" /></label>
        <label className="block"><span className={label}>Respaldo: $ por kilo si un producto no tiene precio en esa lista</span><input type="number" value={form.importePorKg} onChange={(e) => setForm({ ...form, importePorKg: Number(e.target.value) })} className={input} /></label>
