...
import type { EstadoImpresora } from '@/services/zebraBleService'
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        {impresora.estado === 'sin_soporte' ? null : conectada ? (
          ...
          <button type="button" onClick={impresora.onConectar} disabled={impresora.estado === 'conectando'}
            className="inline-flex items-center gap-2 h-11 px-4 rounded-xl bg-accent text-white text-base font-bold touch-manipulation active:opacity-90 disabled:opacity-60">
            <Bluetooth size={18} /> {impresora.estado === 'conectando' ? 'Conectando…' : 'Conectar impresora'}
          </button>
        )}
        <button
          type="button"
