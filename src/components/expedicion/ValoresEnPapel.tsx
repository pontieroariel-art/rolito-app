...
import type { EmpresaTango, TipoRetencion } from '@/types'
...
type ChequeFila = { cobranzaId: string; numeroRecibo?: string; clienteNombre: string; numero: string; bancoNombre: string; fechaAcreditacion: string; importe: number; esEcheq?: boolean; recibido?: boolean; motivoNoEntregado?: string; empresa?: EmpresaTango }
type RetencionFila = { cobranzaId: string; numeroRecibo?: string; clienteNombre: string; tipo: string; nroCertificado: string; importe: number; recibido?: boolean; motivoNoEntregado?: string; empresa?: EmpresaTango }
...
export default function ValoresEnPapel({ cheques, retenciones, decisiones, onDecision, soloLectura = false, titulo }: {
  ...
  const filas: { clave: string; texto: string; detalle: string; importe: number; recibido?: boolean; motivo?: string; empresa?: EmpresaTango }[] = [
    ...cheques.map((ch) => ({
      ...
      importe: ch.importe, recibido: ch.recibido, motivo: ch.motivoNoEntregado, empresa: ch.empresa,
    })),
    ...retenciones.map((re) => ({
      ...
      importe: re.importe, recibido: re.recibido, motivo: re.motivoNoEntregado, empresa: re.empresa,
    })),
  ]
...
function FilaLectura({ f }: { f: { texto: string; detalle: string; importe: number; recibido?: boolean; motivo?: string; empresa?: EmpresaTango } }) {
  ...
  return (
    <div className={`flex items-start gap-3 rounded-lg border px-3 py-2 ${ok ? 'border-[#D3D1C7] bg-white' : 'border-red-200 bg-red-50'}`}>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-gray-900">{f.texto}{f.empresa && <ChipEmpresa empresa={f.empresa} className="ml-2 align-middle" />}</p>
...
function FilaEditable({ f, decision, onDecision }: { f: { texto: string; detalle: string; importe: number; empresa?: EmpresaTango }; decision?: DecisionValor; onDecision: (d: DecisionValor) => void }) {
