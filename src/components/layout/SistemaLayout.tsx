...
// ~95% idénticos que ya habían empezado a divergir (auditoría 2026-08-29, H12);
// ahora cada uno es un wrapper fino que pasa su navegación y sus pocas
// variaciones. El Backoffice queda aparte (chrome propio: sin gestión de
// usuarios, encabezado, logo a /admin).
//
// Variaciones por sistema:
//   subtitulo   — texto tras el rol en la tarjeta de cuenta (sistema / planta).
//   filtrarItem — filtro extra de ítems además de rol + pestañas (ej. el puesto
//                 de cobranza de expedición, que solo ve Cobranzas).
//   guard       — redirect condicional evaluado tras los hooks (mismo caso).
//   children    — pantallas compartidas que no viven anidadas bajo <Outlet/>.

export interface SubtituloCtx {
  user:          UserProfile | null
  multiSistema:  boolean
...
interface SistemaLayoutProps {
  navGroups:    NavGroup[]
  children?:    ReactNode
  subtitulo?:   (ctx: SubtituloCtx) => string | undefined
  filtrarItem?: (to: string) => boolean
  guard?:       ReactNode
}

export default function SistemaLayout({ navGroups, children, subtitulo, filtrarItem, guard }: SistemaLayoutProps) {
  const { user }   = useAuth()
  ...
  const sub = subtitulo?.({ user, multiSistema, sistemaActual })
    ...
    <>
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-gray-500 truncate">
          {user?.rol && ROLE_LABELS[user.rol]}{sub ? ` · ${sub}` : ''}
        </p>
      </div>
