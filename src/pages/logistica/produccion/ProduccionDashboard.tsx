...
// - la impresión sale al confirmar, fuera del camino del toque; si el
//   operario cancela el diálogo el pallet ya existe y se reimprime desde el
//   listado.
export default function ProduccionDashboard() {
  ...
  // imprime la pestaña actual con el ticket renderizado (oculto) y el resto
  // de la página escondida vía CSS de impresión (wrapper "print:hidden").
  useEffect(() => {
    ...
    <>
      ...
      <div className="h-dvh flex flex-col overflow-hidden bg-[#F8F7F2] text-gray-900 print:hidden">
        <main className="flex-1 min-h-0 flex flex-col p-3 gap-3 overflow-hidden">
          <CabeceraPlanta nombre={nombre.split(' ')[0] || 'operario'} planta={PLANTAS[user.planta].label} online={online} onSalir={salir} />
