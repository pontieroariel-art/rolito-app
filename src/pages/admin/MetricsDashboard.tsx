...
export default function MetricsDashboard({ orders }: { orders: Order[] }) {
  ...
  // ── Sección 4: Ranking de clientes del mes ────────────────────────────────
  ...
  // ── Sección 5: Clientes inactivos (+7 días) ───────────────────────────────
    ...
    <section className="space-y-5">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <MetricCard label="Total pedidos"   value={todayOrders.length}        color="text-white"        icon={<Package     size={14} />} />
          <MetricCard label="Pendientes"      value={todayByStatus.pendiente}   color="text-yellow-400"   icon={<Clock       size={14} />} />
          <MetricCard label="Confirmados"     value={todayByStatus.confirmado}  color="text-blue-400"     icon={<CheckCircle2 size={14} />} />
          <MetricCard label="En camino"       value={todayByStatus.en_camino}   color="text-accent"       icon={<Truck       size={14} />} />
          <MetricCard label="Entregados"      value={todayByStatus.entregado}   color="text-success"      icon={<CheckCircle2 size={14} />} />
          <MetricCard label="Cancelados"      value={todayByStatus.cancelado}   color="text-red-400"      icon={<XCircle     size={14} />} />
        </div>
        {todayKg > 0 && (
          <div className="mt-3 bg-success/10 border border-success/20 rounded-xl px-4 py-3 flex items-center gap-3">
            <Weight size={16} className="text-success shrink-0" />
            <p className="text-sm">
              <span className="text-success font-bold text-lg">{todayKg.toLocaleString('es-AR')}</span>
              <span className="text-muted ml-1.5">kg entregados hoy</span>
            </p>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <MetricCard label="Pedidos del mes"     value={monthOrders.length} color="text-white"       icon={<Package    size={14} />} />
          <MetricCard label="Kg del mes"          value={monthKg}            color="text-accent"      icon={<Weight     size={14} />} suffix="kg" />
          <MetricCard label="Clientes activos"    value={monthActiveClients} color="text-blue-400"    icon={<Users      size={14} />} />
          <MetricCard label="Promedio / pedido"   value={avgKgPerOrder}      color="text-purple-400"  icon={<TrendingUp size={14} />} suffix="kg" />
        </div>
      </div>

      {/* ── 3 + 4. Gráfico y Ranking ─────────────────────────────────────────── */}
      <div className="grid md:grid-cols-2 gap-4">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={weeklyData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
              ...
              <Tooltip
                ...
                labelStyle={{ color: '#74a0c8' }}
                itemStyle={{ color: '#00C2FF' }}
                cursor={{ fill: 'rgba(0,194,255,0.06)' }}
              />
              <Bar dataKey="count" name="Pedidos" fill="#00C2FF" radius={[4, 4, 0, 0]} maxBarSize={36} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          ...
          {topClients.length === 0 ? (
            <p className="text-muted text-sm text-center py-10">Sin pedidos este mes</p>
          ) : (
              ...
              {topClients.map((c, i) => (
                <div key={c.clientId} className="flex items-center gap-3">
                  <span className={`w-5 text-xs font-bold shrink-0 text-center ${
                    i === 0 ? 'text-yellow-400' :
                    i === 1 ? 'text-gray-300'   :
                    i === 2 ? 'text-amber-600'   : 'text-muted'
                  }`}>
                    {i + 1}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate leading-tight">{c.name}</p>
                    <p className="text-xs text-muted">{c.count} pedido{c.count !== 1 ? 's' : ''}</p>
                  </div>
      ...
      {/* ── 5. Alertas: clientes inactivos ───────────────────────────────────── */}
      {inactiveClients.length > 0 && (
        <div className="bg-surface border border-yellow-500/20 rounded-xl p-4">
          <p className="text-sm font-semibold mb-3 flex items-center gap-2">
            <AlertTriangle size={14} className="text-yellow-400" />
            <span className="text-yellow-400">Sin pedir hace más de 7 días</span>
          ...
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {inactiveClients.map((c) => (
              <div
                key={c.clientId}
                className="flex justify-between items-center py-1.5 border-b border-border/40 last:border-0 gap-3"
              >
  ...
  return (
    <div className="bg-bg border border-border rounded-xl p-3">
      ...
      <p className={`text-2xl font-bold ${color}`}>
        ...
        {suffix && <span className="text-xs font-normal ml-1 text-muted">{suffix}</span>}
      </p>
    </div>
  )
}
