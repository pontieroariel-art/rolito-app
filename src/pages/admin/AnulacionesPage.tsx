          ...
          <tbody>
            {resueltas.map((a) => (
              <tr key={a.id}>
                ...
                <td className={`${td} text-xs`}>{a.notaCredito?.estado === 'emitida' ? <span>NC {LETRA[a.notaCredito.cbteTipo] ?? ''} {nro(a.notaCredito.puntoVenta, a.notaCredito.numero)}<span className="block text-gray-500">CAE {a.notaCredito.cae}</span></span> : a.notaCredito?.estado === 'incierta' ? <span className="text-amber-700">en revisión en ARCA</span> : '—'}</td>
