...
export default function EtiquetaHeladeraPage() {
  ...
  useEffect(() => {
    ...
    getHeladera(heladeraId).then(async (h) => {
      setHeladera(h)
      if (h) setQrDataUrl(await generateQrDataUrl(`${window.location.origin}/heladeras/ficha/${h.id}`))
    })
  }, [heladeraId])

  useEffect(() => {
    if (heladera && qrDataUrl) {
  ...
  return <ZebraLabel heladera={heladera} qrDataUrl={qrDataUrl} />
}
