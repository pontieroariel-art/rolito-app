...
describe('envasesDeRemito / envasesDeDescarga', () => {
  it('con envases: puntales y aros implícitos, 4 y 1 por pallet de cualquier tipo', () => {
    const e = envasesDeRemito({ palletsCarga: 5, envases: { tarimasMadera: 3, palletsMetal: 2, racks: [12, 15] } })
    expect(e).toEqual({ tarimasMadera: 3, palletsMetal: 2, puntales: 20, aros: 5, racks: [12, 15], origen: 'envases' })
  })

  it('remito viejo (sin envases): palletsCarga se lee como pallets de metal', () => {
    expect(envasesDeRemito({ palletsCarga: 4 })).toEqual({ tarimasMadera: 0, palletsMetal: 4, puntales: 16, aros: 4, racks: [], origen: 'legacy' })
  })
  ...
  it('descarga vieja: completos + parciales + vacíos como pallets de metal con sus implícitos', () => {
    expect(envasesDeDescarga({ palletsCompletos: 3, palletsParciales: 2, palletsVacios: 4 }))
      .toEqual({ tarimasMadera: 0, palletsMetal: 9, puntales: 36, aros: 9, racks: [], origen: 'legacy' })
  })
...
describe('cuadrarEnvases', () => {
  it('diferencia por tipo y racks faltantes por número', () => {
    ...
    expect(c.salieron).toEqual({ tarimasMadera: 3, palletsMetal: 2, puntales: 20, aros: 5, racks: [12, 15, 18] })
    expect(c.volvieron).toEqual({ tarimasMadera: 3, palletsMetal: 2, puntales: 19, aros: 5, racks: [12, 18] })
    expect(c.diferencia).toEqual({ tarimasMadera: 0, palletsMetal: 0, puntales: -1, aros: 0 })
  ...
  it('un día viejo (remito y descarga sin envases) cuadra sin fantasmas', () => {
    ...
    expect(c.diferencia).toEqual({ tarimasMadera: 0, palletsMetal: -1, puntales: -4, aros: -1 })
  ...
  it('suma varios remitos y descargas, y detecta racks sobrantes', () => {
    const c = cuadrarEnvases(
      ...
      [{ envases: { tarimasMadera: 1, palletsMetal: 0, puntales: 4, aros: 1, racks: [1] } }, { envases: { tarimasMadera: 0, palletsMetal: 1, puntales: 4, aros: 1, racks: [2, 9] } }],
    )
    expect(c.salieron).toMatchObject({ tarimasMadera: 1, palletsMetal: 1, puntales: 8, aros: 2, racks: [1, 2] })
