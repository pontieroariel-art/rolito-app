...
describe('envasesDeRemito / envasesDeDescarga', () => {
  it('con envases: 4 puntales y 1 sombrero por pallet de cualquier tipo, 1 aro solo por tarima de madera', () => {
  ...
  it('descarga vieja: completos + parciales + vacíos como pallets de metal con sus implícitos', () => {
    expect(envasesDeDescarga({ palletsCompletos: 3, palletsParciales: 2, palletsVacios: 4 }))
      .toEqual({ tarimasMadera: 0, palletsMetal: 9, puntales: 36, aros: 0, sombreros: 9, racks: [], origen: 'legacy' })
  })

  it('descarga nueva se toma tal cual (puntales y aros contados sueltos)', () => {
    const e = envasesDeDescarga({ envases: { tarimasMadera: 3, palletsMetal: 2, puntales: 19, aros: 5, sombreros: 4, racks: [12, 18] } })
    expect(e).toMatchObject({ tarimasMadera: 3, palletsMetal: 2, puntales: 19, aros: 5, sombreros: 4, racks: [12, 18], origen: 'envases' })
    // Descarga anterior al sombrero: se lee como 0 contados.
    expect(envasesDeDescarga({ envases: { tarimasMadera: 1, palletsMetal: 0, puntales: 4, aros: 1, racks: [] } }).sombreros).toBe(0)
  })
})
...
describe('cuadrarEnvases', () => {
  it('diferencia por tipo y racks faltantes por número', () => {
    const c = cuadrarEnvases(
      ...
      [{ envases: { tarimasMadera: 3, palletsMetal: 2, puntales: 19, aros: 5, sombreros: 5, racks: [18, 12] } }],
    )
    expect(c.salieron).toEqual({ tarimasMadera: 3, palletsMetal: 2, puntales: 20, aros: 3, sombreros: 5, racks: [12, 15, 18] })
    expect(c.volvieron).toEqual({ tarimasMadera: 3, palletsMetal: 2, puntales: 19, aros: 5, sombreros: 5, racks: [12, 18] })
    expect(c.diferencia).toEqual({ tarimasMadera: 0, palletsMetal: 0, puntales: -1, aros: 2, sombreros: 0 })
  ...
  it('un día viejo (remito y descarga sin envases) cuadra sin fantasmas', () => {
    ...
    expect(c.diferencia).toEqual({ tarimasMadera: 0, palletsMetal: -1, puntales: -4, aros: 0, sombreros: -1 })
  ...
  it('suma varios remitos y descargas, y detecta racks sobrantes', () => {
    const c = cuadrarEnvases(
      ...
      [{ envases: { tarimasMadera: 1, palletsMetal: 0, puntales: 4, aros: 1, sombreros: 1, racks: [1] } }, { envases: { tarimasMadera: 0, palletsMetal: 1, puntales: 4, aros: 0, sombreros: 1, racks: [2, 9] } }],
    )
    expect(c.salieron).toMatchObject({ tarimasMadera: 1, palletsMetal: 1, puntales: 8, aros: 1, sombreros: 2, racks: [1, 2] })
...
describe('parseRacks / describir', () => {
  ...
  it('describe lo que hay y omite los ceros', () => {
    expect(describirEnvases({ tarimasMadera: 3, palletsMetal: 0, puntales: 12, aros: 1, sombreros: 3, racks: [15, 12] })).toBe('3 madera · 12 puntales · 1 aro · 3 sombreros · racks Nº 12, 15')
